...
dataset_tidy |> kbl(caption = "Table 1: Dummy data derived for pilot test") |> kable_styling() 
```

## Compute plot biomass
...
dataset_tidy = dataset_tidy |>
  dplyr::group_by(species_j, stratum_i, plot_sp) |>
  dplyr::mutate(vji_sp_m3 = sum(volume))
...
```{r}
#| layout-ncol: 3
psych::describe(dataset_tidy)
plot_vol = lm(plot_sp ~ volume, data = dataset_tidy)
hist(dataset_tidy$vji_sp_m3, main="Volume")
hist(dataset_tidy$volume, main="")
lattice::histogram(plot_sp ~ volume, data = dataset_tidy)

wilcox.test(dataset_tidy$vji_sp_m3) # p<0.006
wilcox.test(dataset_tidy$volume) # p<0.0002
wilcox.test(dataset_tidy$plot_sp) # p<0.0002

# Inspect univariate trends in residual variance 
olsrr::ols_test_breusch_pagan(plot_vol) # p=0.0012
lattice::densityplot(dataset_tidy$volume, type = "density")
lattice::densityplot(dataset_tidy$plot_sp, type = "density")
plot(plot_sp ~ volume, data = dataset_tidy, 
 main="Linear function showing negative correlation")
abline(plot_vol, col = "red")
```

## Simulation Regime

The `LGOCV` acronym stands for "leave one group out cross validation". We must select the % of test data that is set out from the build upon which the model will be repeatedly trained. Repeat n number of times)

```{r}
monte_carlo = trainControl(
  method = "LGOCV",
  p      = 0.8, 
  number = 10000) # how many simulations
